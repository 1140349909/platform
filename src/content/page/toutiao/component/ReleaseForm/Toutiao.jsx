...
export default class Toutiao extends AuthComponentBase {
        ...
        if (this.state.status == 'not') {
            ...
            if (_.has(this.props.form.getFieldsValue(), 'toutiao.imgId.file.response.data')) {
                if (!this.props.form.getFieldsValue().toutiao.imgId.file.response.data && _.has(this.props.data.toutiao, 'imgUrl')) {
                    ...
                    mediaUrl = getMediaUrl(this.props.form.getFieldsValue().toutiao.imgId.file.response.data);
                }
            } else {
    ...
    uploadChange = (data) => {
        ...
        if (status == 'uploading') {
            ...
            this.setState({
                mediaUrl: getMediaUrl(data.file.response.data),
                status: 'done',
            });
    ...
    render() {
        ...
        const toutiaoTitle = getFieldDecorator('toutiao.title', {rules: this.getRules().concat([
            {min:5}
        ])});
