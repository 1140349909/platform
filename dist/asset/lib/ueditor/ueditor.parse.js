...
(function () {
    ...
    // TODO:img的处理
    UE.parse.register('img', function (utils) {
        var imgs = this.root.getElementsByTagName('img');
        if (imgs.length) {
            utils.each(imgs, function (img) {
                var alt = img.getAttribute('alt');
                var mediaId = alt.match(/^lk-(\w+)$/)
                if (mediaId.length == 2 && mediaId[1]) {
                    mediaId = mediaId[1]
                }
                img.src = 'http://api.sit.vveshow.com/iloka/api/v1/w1/linkin/media/image/' + mediaId;
            });
        }
